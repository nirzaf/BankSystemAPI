namespace CentralApi.Controllers
{
    ...
    public class ReceiveTransactionsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromBody] string data)
        {
            ...
            var bank = await banksService.GetBankAsync<BankServiceModel>(model.DestinationBankName,
                model.DestinationBankSwiftCode,
                model.DestinationBankCountry);

            if (bank == null)
            {
                return NotFound(string.Format(BankNotFound, model.DestinationBankName));
            }

            var sendModel = mapper.Map<SendTransactionModel>(model);
            var encryptedAndSignedData =
                TransactionHelper.SignAndEncryptData(sendModel, configuration.Key, bank.ApiKey);
            var client = new HttpClient();
            var response = await client.PostAsJsonAsync(bank.ApiAddress, encryptedAndSignedData);
            if (response.StatusCode != HttpStatusCode.OK)
            {
                return BadRequest(string.Format(BankRefusedTheRequestMessage, model.DestinationBankName));
            }
