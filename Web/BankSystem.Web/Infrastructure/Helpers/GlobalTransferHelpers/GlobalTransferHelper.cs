namespace BankSystem.Web.Infrastructure.Helpers.GlobalTransferHelpers
{
    ...
    public class GlobalTransferHelper : IGlobalTransferHelper
    {
        ...
        public async Task<GlobalTransferResult> TransferMoneyAsync(GlobalTransferDto model)
        {
            ...
            var account = await bankAccountService
            ...
            bool remoteSuccess = await ContactCentralApiAsync(submitDto);
            if (!remoteSuccess)
            {
                return GlobalTransferResult.GeneralFailure;
            }
            ...
            bool success = await moneyTransferService.CreateMoneyTransferAsync(serviceModel);
        ...
        {
            var encryptedData = SignAndEncryptData(model);

            var client = new HttpClient();
            var response = await client.PostAsJsonAsync(
                string.Format(CentralApiTransferSubmitUrlFormat, bankConfiguration.CentralApiAddress),
                encryptedData);
        ...
        private string SignAndEncryptData(CentralApiSubmitTransferDto model)
        {
            ...
            RsaExtensions.FromXmlString(rsa, bankConfiguration.Key);
            ...
            {
                RsaExtensions.FromXmlString(encryptionRsa, bankConfiguration.CentralApiPublicKey);
            ...
            var json = new
            {
                ...
                BankSwiftCode = bankConfiguration.UniqueIdentifier,
                BankCountry = bankConfiguration.Country,
